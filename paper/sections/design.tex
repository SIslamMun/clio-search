...
Nine connectors mediate access to heterogeneous backends: local filesystem, S3-compatible object stores, Qdrant vector database, Neo4j graph database, HDF5 (via h5py), NetCDF (via xarray), IOWarp, NDP catalog (via MCP), and a key-value log store. IOWarp~\cite{iowarp2024} is an NSF-funded open-source context management platform for HPC and data-intensive workloads. Its Context Transfer Engine (CTE) organizes data as \emph{blobs} grouped under \emph{tags}, with programmable buffering across memory and storage tiers (RAM, file-backed, custom block devices). CLIO Kit, the platform's AI tooling layer, exposes 16 MCP servers with 150+ tools spanning scientific data formats (HDF5, Parquet, ADIOS), HPC job management (Slurm), and dataset discovery (NDP). Each connector declares a capability set specifying which retrieval branches it supports, enabling the coordinator to skip unsupported branches rather than fail. The IOWarp connector indexes scientific metadata from CTE blobs at ingest time and answers cross-unit queries against the DuckDB index; the NDP connector discovers and indexes datasets from the National Data Platform~\cite{ndp2024} via the Model Context Protocol~\cite{islam2025aidriven}. Every pipeline stage emits structured trace events for observability.
