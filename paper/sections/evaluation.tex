...
retrieval. We also validate on 288 real NOAA observatory
...
\bottomrule
\multicolumn{3}{@{}l}{\scriptsize $^\dagger$Now in registry; no query targets hPa.}
\end{tabular}
\renewcommand{\arraystretch}{1.15}
\end{table}
...
averages 25.6~tool calls per query and NDP-MCP averages 7.6.
...
gains are specific to cross-unit matching. R@5 is low across all methods (0.07--0.19)
because the 210-document corpus produces approximately 42 relevant
documents per query on average, making recall at $K{=}5$ structurally
bounded.
...
We validate this on IOWarp blobs at scale: the rich variant
(2.5~meas/blob) achieves 0.01\% inspection at 500K (90\,ms profiling);
the sparse variant (0.75~meas/blob) achieves 0.05\% at 100K (47\,ms).
...
IOWarp's BlobQuery API returns all blobs matching a tag regex with
no content filtering; CLIO addresses this by indexing scientific
metadata from blobs at ingest time (Sec.~3.1). We store synthetic
scientific blobs across four tags
...
scales to 500K; sparse (0.75~meas/blob) validates adaptive strategy
at 100K. CLIO returns top-50 unit-converted matches; BlobQuery
returns all blobs unfiltered.}
\label{tab:iowarp}
\footnotesize
...
5K   & 348\,s     & 16\,ms & 12{,}500  & 50 & 1{,}250 \\
...
filter by content (Table~\ref{tab:iowarp}, Fig.~\ref{fig:iowarp-scaling}). For the rich-metadata variant, the inspection rate falls
from 5.0\% at 1K to \textbf{0.01\% at 500K} ($500\times$ reduction in
inspection rate). Corpus profiling scales sub-linearly: 500$\times$
...
inspection rate (right). Rich metadata (2.5 meas/blob) scales to 500K;
sparse metadata (0.75 meas/blob) demonstrates adaptive strategy at
100K. Inspection drops to 0.01\% at 500K.}
...
Our distributed experiments use 4~GH200 nodes on DeltaAI with DuckDB
shards on the Lustre parallel filesystem. We scale \emph{data}, not
nodes: weak scaling grows the corpus proportionally
(625K$\to$2.5M), achieving 0.94 efficiency at $4\times$. Each shard
is independent; coordination overhead stays under 3\,ms. This
generalizes to more nodes without algorithmic changes, though
validating at hundreds of nodes remains future work.
P@5 is a mean over queries without confidence intervals; the
40-query cross-unit and 80-query full benchmarks provide reasonable
estimates but not statistical significance. The controlled benchmark
is synthetic; the NOAA evaluation (288~real documents) partially
addresses ecological validity.
