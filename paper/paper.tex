...
We further evaluate the agentic loop across four LLM providers spanning cloud and local inference (Table~\ref{tab:llm_providers}).

\begin{table}[!t]
\centering
\caption{LLM provider comparison for query rewriting. Latency and retrieval quality measured on 10 representative queries.}
...
\begin{tabular}{@{}l l c c c c@{}}
\toprule
\textbf{Provider} & \textbf{Type} & \textbf{Lat.(s)} & \textbf{P@5} & \textbf{MRR} & \textbf{Cost} \\
\midrule
Ollama (Gemma) & Local  & 0.76 & 0.90 & 1.00 & \$0 \\
\rowcolor{gray!5}
Gemini 1.5 Flash & Cloud  & 0.11 & 0.90 & 1.00 & \$0 \\
Claude Sonnet & Session & 8.24 & 0.86 & 0.88 & \$0.008 \\
\rowcolor{gray!5}
Fallback (SI exp.) & None & 0.00 & 0.86 & 0.88 & \$0 \\
\bottomrule
\end{tabular}
\renewcommand{\arraystretch}{1.15}
\end{table}

Ollama (local, zero cost) and Gemini Flash (cloud, sub-second latency) both achieve P@5 of 0.90 with perfect MRR, outperforming both the Claude Agent SDK (8.2s latency, session-based) and the no-LLM fallback. The system supports nine additional providers---Llama~3.1, Qwen~2.5, DeepSeek-R1, Mistral, Gemini~Pro, Claude Haiku/Opus, LM~Studio, and vLLM---through a unified OpenAI-compatible interface, enabling users to select the latency--quality--cost trade-off appropriate to their deployment. The key finding is that local inference via Ollama matches cloud quality at zero marginal cost, making the agentic loop practical for HPC environments where outbound network access may be restricted.
