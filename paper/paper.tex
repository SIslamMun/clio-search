...
\textbf{Offline conversion tools.} Unit conversion itself is well-established: UDUNITS-2~\cite{udunits2} provides a C library for converting between physical units, and is embedded in NetCDF toolkits (e.g., \texttt{cf\_units}). GNU Units and the R \texttt{units} package offer similar offline conversion. These tools convert \emph{data values} during preprocessing or analysis---they do not operate at retrieval time, do not index measurements, and expose no search interface. A scientist must already know which files contain relevant data before applying UDUNITS. Our contribution is orthogonal: we bring arithmetic conversion \emph{inside} the retrieval pipeline so that the search itself bridges unit heterogeneity.

\textbf{Gap.} To our knowledge, no existing \emph{retrieval system} integrates arithmetic SI conversion as a first-class retrieval operator. Offline conversion tools (UDUNITS, GNU Units) require the user to locate relevant data first. String normalization and learned embeddings operate within retrieval but cannot perform the required arithmetic. No system combines quantity-aware retrieval with formula normalization or federated search across HPC backends.
