...
\textbf{Agentic retrieval.} The SoK on Agentic RAG \cite{sok_agentic_rag2026} provides the first systematization of knowledge in this space. Context-1 \cite{context1_2026} demonstrates a trained 20B-token agentic search agent with self-editing context management. PRISM \cite{prism2025} decomposes retrieval into precision and recall subagents (Analyzer, Selector, Adder) that iteratively refine evidence sets, outperforming baselines on multi-hop QA. Agentic-R \cite{agenticr2026} co-trains retrievers with global answer-correctness rewards rather than local relevance, achieving 10--13\% gains on multi-hop benchmarks. CLADD \cite{cladd2025} augments collaborative drug-discovery agents with domain knowledge graph operators. SciAgent \cite{sciagent2025} coordinates specialist worker agents under a hierarchical controller. ScienceClaw \cite{scienceclaw2026} deploys a 300+ skill agent swarm with immutable artifact provenance. These systems advance orchestration, learned retrieval, and context management. To our knowledge, none incorporates science-aware numerical operators---arithmetic dimensional conversion or formula normalization---into the retrieval pipeline itself. Our work is complementary: the agentic loop and learned reranking techniques from PRISM and Agentic-R could be layered on top of our science-aware operators.
...
String normalization cannot perform the arithmetic required to recognize that 200~kPa equals 200000~Pa, yielding only P@5 of 0.38. Our full pipeline converts all measurements to canonical SI base units at index time and issues direct range predicates at query time, achieving P@5 of 0.99 and MRR of 0.99---an absolute improvement of 0.56 P@5 and 0.50 MRR over the strongest text baseline.
...
\caption{Cross-unit retrieval P@5 across six configurations. Science-aware operators (rightmost) nearly triple the best text baseline.}
\label{fig:crossunit}
\end{figure}
