...
We evaluate CLIO against three claims: (1)~CLIO's agentic pipeline
reduces LLM token consumption and wall time for scientific data
discovery, (2)~CLIO's science-aware operators provide cross-unit and
formula-matching capabilities that standard retrieval systems lack, and
(3)~CLIO integrates with IOWarp's blob storage to enable unit-aware
search at scale. Experiments L1--L8 run on a single workstation;
D1--D2 scale to NCSA DeltaAI (GH200 Grace Hopper).
...
\end{tabular}
\vspace{0.5mm}
{\footnotesize All three paths use \texttt{claude-sonnet-4-20250514}.}
\renewcommand{\arraystretch}{1.15}
\end{table}

Table~\ref{tab:l1-results} shows that CLIO reduces total token
consumption by \textbf{52.08\%} and wall-clock time by \textbf{91.27\%}
compared to the Claude Native Agent baseline, while answering all
10~queries. The native agent and NDP-MCP consume nearly identical tokens
($\sim$989K) through different mechanisms: the native agent spawns
9~sub-agents for parallel web research (563K main + 426K sub-agent
tokens), while NDP-MCP injects full catalog JSON into the LLM context
(989K main tokens, zero sub-agents). CLIO avoids both by indexing 341
NDP datasets (359 chunks, 1{,}945 measurements) into a local DuckDB
store and returning only compact ranked citations ($\sim$2K tokens per
query).

NDP-MCP reduces wall time by 65.68\% over the native agent
(17.4~min vs.\ 50.8~min) by querying the NDP API directly rather than
searching the web, but achieves no token savings because the raw catalog
text still enters the prompt. CLIO reduces time by a further 74.57\%
over NDP-MCP (4.4~min vs.\ 17.4~min) and tokens by 52.06\% because the
agent receives only 10~ranked citations per query rather than 25~full
dataset descriptions. The CLIO agent uses exactly 2~tool calls per query
(one \texttt{ToolSearch} + one \texttt{clio\_search}) regardless of
query complexity.
...
number of active workers (1, 2, 4). Each worker holds $2.5\text{M}/4$
...
Indexing throughput (11--13 blobs/s) reflects the Docker overlay
filesystem constraint; the Chimaera runtime and CTE operations
themselves run at 5{,}000+ blobs/s write throughput. The IOWarp
connector (\texttt{connectors/iowarp/connector.py}) implements
\texttt{NamespaceConnector} with full \texttt{LexicalSearchCapable},
\texttt{VectorSearchCapable}, \texttt{ScientificSearchCapable}, and
\texttt{CorpusProfileCapable} protocols, making IOWarp a first-class
CLIO backend alongside filesystem, S3, HDF5, and NetCDF connectors.
...
\subsection{Part 2: NDP-MCP Integration}
\label{sec:ndp}

The L1 experiment (Table~\ref{tab:l1-results}) evaluates CLIO's
integration with the National Data Platform via the MCP protocol. CLIO
discovers 341 datasets from NDP's CKAN API (hosting NOAA, NASA, DOE,
USGS, and 70+ organizations), indexes them into 359 chunks with 1{,}945
extracted measurements (42.1\% metadata density), and answers all
10~queries at 52\% fewer tokens than direct NDP-MCP access.

\textbf{Cross-unit gap.} NDP's CKAN API performs string matching---it
returns 0~results for ``fahrenheit'' and cannot bridge SI prefixes.
CLIO's dimensional conversion handles this when indexed data contains
extractable measurements. On catalog metadata (42.1\% density), the
agentic layer profiles the namespace, detects partial metadata, and
activates scientific operators selectively.

\textbf{Branch selection.} Across 10~queries, CLIO activates only the
branches that can contribute results. For semantic queries (glacier,
wildfire), only lexical and vector branches activate (scientific
skipped---no numeric operators). For cross-unit queries (temperature,
pressure, wind), all three branches activate, with the scientific branch
returning empty when NDP descriptions lack raw numeric values. This
adaptive selection avoids unnecessary computation without losing
coverage.

\subsection{Part 3: IOWarp Scale Projection}
\label{sec:scale}

We project CLIO's savings to IOWarp's target scale of 10~billion blobs.
Corpus profiling completes in 14--22\,ms via DuckDB aggregation
(Table~\ref{tab:uc3-iowarp}), remaining sub-linear as corpus size grows
from 1K to 10K blobs. At 10K blobs, CLIO inspects 0.5\% of the
corpus per query; at 10B blobs, brute-force scanning would cost an
estimated 4~trillion tokens per query (\$12M at \$3/1M tokens). CLIO's
index-backed filtering reduces this to the small subset of blobs whose
canonical SI values fall within the query range, regardless of the unit
the scientist uses to express the constraint.

\subsection{Part 4: Metadata-Adaptive Discovery}
\label{sec:adaptive}

We test CLIO's ability to adapt when datasets vary in metadata quality.
We create three corpus variants from 210 documents: \textbf{rich}
(87.5\% metadata density, 2{,}674 measurements), \textbf{sparse}
(measurements stripped, 10.3\% density), and \textbf{no-metadata} (all
numbers removed, 4.4\% density). CLIO correctly classifies each: rich
$\to$ ``metadata\_rich,'' sparse $\to$ ``default,'' no-metadata $\to$
``metadata\_sparse.'' On 10~cross-unit queries, only the rich corpus
produces hits via SI conversion; the other two correctly fall back to
text-based retrieval. This confirms that unit conversion is one
capability the agent uses \emph{when metadata exists}---the agent knows
when to use it and when to fall back.
