...
We evaluate on a controlled benchmark of 210 documents across seven scientific domains with deliberate cross-unit variation and 80 queries spanning cross-unit, same-unit control, formula, and multi-constraint conditions. On cross-unit queries, the dimensional operator achieves 0.99 P@5 and 0.99 MRR where the strongest text baseline scores 0.43 P@5 and 0.49 MRR. Formula normalization provides complementary value through strict normalized matching, and the two operators compose independently. Federated search nearly doubles coverage from 24.4\% to 47.2\% across namespaces. An ablation study confirms that the science-aware branch contributes the largest marginal gain (+0.63 P@5). All code and benchmarks are open-source.
\end{abstract}
...
\rowcolor{gray!5}
Dense-only         & 0.30 & 0.05 & 0.07 & 0.43 \\
Hybrid             & 0.43 & 0.07 & 0.10 & 0.49 \\
\rowcolor{gray!5}
String norm.       & 0.38 & 0.04 & 0.07 & 0.42 \\
LLM-assisted       & 0.40 & 0.08 & 0.12 & 0.47 \\
\midrule
\rowcolor{gray!15}
...
Table~\ref{tab:crossunit} reports results on cross-unit queries. R@5 is low across all methods because the 210-document corpus produces a high average number of relevant documents per query (approximately 42), making recall at $K{=}5$ structurally bounded. P@5 and MRR are the more meaningful metrics for this evaluation. The three text-oriented baselines---BM25, dense, and hybrid---retrieve only documents whose surface text contains the same unit string as the query. A query for ``200 kPa'' does not lexically match a document containing ``200000 Pa,'' and the embedding model has no mechanism to learn that these denote identical physical quantities. The strongest text baseline (Hybrid) achieves P@5 of 0.43 and MRR of 0.49.

The string normalization baseline canonicalizes numeric tokens before indexing but cannot perform the arithmetic conversion required to recognize that 200 kPa equals 200000 Pa---it operates on token forms, not physical semantics, yielding only P@5 of 0.38. Our full pipeline bypasses this limitation entirely: at index time, all measurements are converted to canonical SI base units and stored in the \texttt{scientific\_measurements} table; at query time, the dimensional operator converts the query bounds to the same canonical space and issues a direct range predicate. This yields P@5 of 0.99 and MRR of 0.99 on cross-unit queries, representing an absolute improvement of 0.56 points in P@5 and 0.50 points in MRR over the strongest text baseline.
...
On the same-unit control queries, the A-through-D progression shows all configurations within 0.05 P@5 of each other (range 0.30--0.45), confirming that the science-aware operators' value is specific to the cross-unit retrieval scenario they were designed for.
...
For deployments where LLM inference is unavailable or latency-constrained, the system falls back to a deterministic rewriting strategy that expands queries with SI unit variants. On our benchmark, the non-LLM fallback achieves comparable P@5 and MRR, confirming that the deterministic fallback is sufficient when science-aware operators are the primary source of retrieval gains.

We further evaluate the agentic loop across 11 LLM configurations spanning four inference backends: Ollama (local GPU), Google Gemini (cloud API), Claude Agent SDK (session-based), and a no-LLM fallback (Table~\ref{tab:llm_providers}).

\begin{table*}[!t]
\centering
\caption{LLM provider benchmark for agentic query rewriting. 11 configurations across 4 backends, evaluated on 10 representative queries. Retrieval P@5 and MRR measure end-to-end quality after rewriting. The system additionally supports LM~Studio, vLLM, OpenAI, Together~AI, and Groq via a unified OpenAI-compatible interface.}
\label{tab:llm_providers}
\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{l l l c c c c c}
\toprule
\textbf{Backend} & \textbf{Model} & \textbf{Inference} & \textbf{Params} & \textbf{Lat.(s)} & \textbf{P@5} & \textbf{MRR} & \textbf{Cost/q} \\
\midrule
\multirow{4}{*}{Ollama (local)} & Llama 3.2          & GPU & 3B  & 4.93  & 0.90 & 0.95 & \$0 \\
                                & Llama 3.1          & GPU & 8B  & 10.84 & 0.90 & 0.95 & \$0 \\
\rowcolor{gray!5}
                                & Mistral 7B         & GPU & 7B  & 10.52 & 0.90 & 0.95 & \$0 \\
                                & Qwen 2.5           & GPU & 14B & 18.08 & 0.90 & 1.00 & \$0 \\
\midrule
\multirow{3}{*}{Google Gemini}  & Gemini 2.0 Flash   & Cloud API & --- & 0.09 & 0.90 & 1.00 & $\sim$\$0 \\
\rowcolor{gray!5}
                                & Gemini 1.5 Flash   & Cloud API & --- & 0.06 & 0.90 & 1.00 & $\sim$\$0 \\
                                & Gemini 1.5 Pro     & Cloud API & --- & 0.05 & 0.90 & 1.00 & $\sim$\$0 \\
\midrule
\multirow{3}{*}{Claude (SDK)}   & Claude Sonnet      & Session   & --- & 8.56 & 0.86 & 0.88 & \$0.008 \\
\rowcolor{gray!5}
                                & Claude Haiku       & Session   & --- & 8.79 & 0.86 & 0.88 & \$0.003 \\
                                & Claude Opus        & Session   & --- & 9.34 & 0.86 & 0.88 & \$0.015 \\
\midrule
\rowcolor{gray!10}
No LLM                          & SI expansion       & None      & 0   & 0.00 & 0.86 & 0.88 & \$0 \\
\bottomrule
\end{tabular}
\renewcommand{\arraystretch}{1.15}
\end{table*}

Three findings emerge. First, local Ollama inference matches cloud quality (P@5 = 0.90 across all four models) at zero marginal cost, making the agentic loop practical for HPC environments where outbound network access may be restricted. Qwen~2.5~(14B) achieves perfect MRR despite higher latency. Second, Gemini Flash models achieve sub-100ms rewriting latency with perfect MRR---an order of magnitude faster than local inference---suitable for interactive deployments. Third, the no-LLM fallback (deterministic SI unit expansion) achieves 96\% of the best LLM-assisted P@5, confirming that the science-aware operators are the primary source of retrieval gains; the LLM provides incremental improvement through query expansion rather than fundamental capability.
...
\textbf{Unsupported units.} Queries involving units outside our 13-unit conversion table (e.g., temperature in Rankine, force in dynes, or compound units such as N/m$^2$) produce no dimensional matches and fall back to lexical and vector retrieval only. The system degrades gracefully---it does not produce incorrect matches for unsupported units---but the science-aware advantage is lost. On the v2 benchmark, 32 atmospheric documents contain hPa (unsupported unit) and 25 materials documents contain degC. The system correctly falls back to lexical and vector retrieval for these documents, producing no false positive dimensional matches.
...
The results validate the core hypothesis on a benchmark of 210 documents across seven scientific domains with 80 queries. Dimensional-conversion operators achieve P@5 of 0.99 and MRR of 0.99 on cross-unit queries, where the strongest text baseline (Hybrid) scores only 0.43 P@5 and 0.49 MRR. The ablation study isolates this gain: adding science-aware operators produces a +0.63 jump in P@5 (from 0.36 to 0.99), confirming that cross-unit matching is the dominant retrieval bottleneck. Formula normalization provides complementary value through strict normalized matching that trades recall for precision---an honest design choice favoring reliable matches over loose text overlap. The two operators compose independently; neither subsumes the other. Federated search nearly doubles coverage from 24.4\% to 47.2\% across namespaces, while capability negotiation ensures that adding a backend without scientific measurement support does not degrade results from backends that have it. Multi-hop agentic retrieval achieves P@5 of 0.98 and MRR of 0.94, converging at hop 1--2 on this benchmark; the multi-hop loop is designed for more complex real-world queries where initial formulations are underspecified. These gains matter because AI agents are now orchestrating scientific workflows end-to-end, from hypothesis generation through experiment execution, yet their retrieval layers treat measurements as opaque strings.
