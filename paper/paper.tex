...
\textbf{Extraction failures.} The regex-based measurement extractor may miss quantities expressed in non-standard formats. Expressions such as ``$2 \times 10^3$ Pa'' or ``two hundred kilopascals'' are not captured by the current pattern, which expects a numeric literal adjacent to a unit abbreviation. On our benchmark, the extractor achieves [TBD]\% precision and [TBD]\% recall on measurement identification.
