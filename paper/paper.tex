...
Scientific computing generates data at unprecedented scale, yet an estimated 60--80\% of that data is never reused after creation---not because it is valueless, but because it is unsearchable. When a scientist at a DOE facility cannot find that a 200~kPa combustion simulation already exists---stored as ``200000~Pa'' on a different filesystem---they re-run it, wasting allocations that cost millions of dollars per year per leadership-class machine. The consequences of unit mismatch extend beyond inconvenience: NASA's loss of the \$327M Mars Climate Orbiter, caused by a pound-seconds versus newton-seconds confusion, remains the canonical example. The FAIR data principles now mandated by NSF, NIH, and DOE require that data be \emph{Findable}---yet unit heterogeneity across storage tiers systematically violates this requirement. A query for pressures between 190--360~kPa should match ``200000~Pa'' in an HDF5 attribute and ``0.2~MPa'' in an S3-hosted dataset. These describe identical pressure. Every retrieval system treats them as unrelated strings.
