...
\textbf{Benchmark construction.} We construct a controlled benchmark of 210 documents across seven scientific domains: materials science (40 documents), atmospheric science (40), fluid dynamics (40), chemistry (30), HPC simulation metadata (30), cross-domain mixed (20), and negative controls (10). Each document contains 100--400 words of realistic scientific prose---experiment reports, simulation summaries, and lab notebook entries---with deliberate unit heterogeneity: the same physical quantities are expressed in different SI prefixes across documents. Ten negative-control documents contain no extractable measurements. Documents with unsupported units (hPa, degC) test graceful degradation to lexical and vector retrieval. We define 80 queries: 40 cross-unit queries requiring dimensional conversion, 20 same-unit control queries, 10 formula queries, and 10 multi-constraint queries. Ground truth is generated by automated parsing and unit-converted range matching. The same-unit control set establishes that observed differences are attributable to unit mismatch rather than content difficulty.
...
For deployments where LLM inference is unavailable or latency-constrained, the system falls back to a deterministic rewriting strategy: the rewriter extracts entities and measurement tuples from first-hop results and appends them as structured filters to the second-hop query. On our benchmark, the non-LLM fallback achieves comparable P@5 and MRR, as the SI expansion heuristic catches the unit variants that the science-aware operators already resolve in a single pass. This confirms that the deterministic fallback is sufficient when the science-aware operators are the primary source of retrieval gains.
