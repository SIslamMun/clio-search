...
We present clio-agentic-search, a retrieval engine that introduces \emph{science-aware operators}---deterministic retrieval primitives that execute as first-class branches alongside BM25 and dense vector search. A dimensional-conversion operator canonicalizes physical quantities to base SI units via explicit arithmetic (kPa~$\times$~$10^3$~=~Pa), guaranteeing correctness by construction across 16 supported units spanning pressure, temperature (°C/°F~$\to$~K), velocity, length, mass, and time. A formula-normalization operator matches equations across notation variants. These operators run within a federated architecture that queries six backend types through per-backend capability negotiation, with an LLM-driven agentic loop for iterative multi-hop refinement.

We evaluate on two benchmarks. On a controlled corpus of 210 documents across seven scientific domains with 80 queries, the dimensional operator achieves 0.99 P@5 and 0.99 MRR where the strongest text baseline scores 0.43 P@5. An ablation study confirms the science-aware branch contributes the largest marginal gain (+0.63 P@5). We further validate on 288 real documents from the NOAA GHCN-Daily observatory dataset---documents report only metric units while queries use imperial units---where our pipeline achieves 0.70 P@5 against 0.38 for hybrid baselines (+87\%). Formula normalization and federated search provide complementary gains. All code and benchmarks are open-source.
\end{abstract}
...
\begin{enumerate}
\item \textbf{Dimensional-conversion retrieval.} We canonicalize physical quantities to base SI units via arithmetic multiplication (e.g., kPa $\times$ $10^3$ = Pa) across 16 units in 6 domains, with correctness guaranteed by construction, provided units are correctly extracted and conversion factors are complete.
...
\rowcolor{gray!5}
Temperature & degC, degF, K  & K   & $+273.15$, $\times\frac{5}{9}\!+\!255.37$, 1 \\
\bottomrule
\end{tabular}
\renewcommand{\arraystretch}{1.15}
\end{table}
...
\textbf{Unsupported units.} Queries involving units outside our supported conversion table (e.g., temperature in Rankine, force in dynes, or compound units such as N/m$^2$) produce no dimensional matches and fall back to lexical and vector retrieval only. The system degrades gracefully---no incorrect matches are produced for unsupported units---but the science-aware advantage is lost. Our v2 implementation covers 16 units including temperature (degC/degF $\to$ K), velocity, pressure, mass, length, and time.
