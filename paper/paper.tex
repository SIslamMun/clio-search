...
\begin{table*}[!t]
\centering
\caption{Capability comparison across retrieval and scientific data systems.}
\label{tab:comparison}
\footnotesize
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{@{}l ccccc ccccc@{}}
\toprule
 & \multicolumn{5}{c}{\textit{Our Contributions}} & \multicolumn{5}{c}{\textit{General}} \\
\cmidrule(lr){2-6} \cmidrule(lr){7-11}
\textbf{System} & \rotatebox{65}{SI conv.} & \rotatebox{65}{Formula} & \rotatebox{65}{Sci. branch} & \rotatebox{65}{Federated} & \rotatebox{65}{HDF5/NC} & \rotatebox{65}{Agentic} & \rotatebox{65}{Qty. extr.} & \rotatebox{65}{Papers} & \rotatebox{65}{Data} & \rotatebox{65}{LLM} \\
\midrule
\rowcolor{gray!10}
...
\end{tabular}
\\[2pt]
{\scriptsize \cmark{} = supported, \xmark{} = not supported, $\circ$ = partial.}
\end{table*}
...
\begin{table}[!t]
\centering
\caption{SI conversion table (5 domains, 13 units).}
...
\begin{table}[!t]
\centering
\caption{DuckDB schema (8 tables).}
\label{tab:schema}
\scriptsize
...
\midrule
documents       & Corpus membership  & URI, namespace, SHA-256 \\
chunks          & Text segments      & text, doc FK, position \\
embeddings      & Dense vectors      & 384-dim float, chunk FK \\
metadata        & Annotations        & key-value per doc/chunk \\
file\_index     & Change detection   & path, SHA-256, mtime \\
lexical\_post.  & BM25 retrieval     & token$\to$chunk, TF \\
\rowcolor{gray!5}
sci\_meas.      & Dimensional search & raw/canon. value+unit \\
\rowcolor{gray!5}
sci\_formulas   & Formula matching   & raw, normalized form \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!t]
\centering
\caption{Cross-unit retrieval ($K{=}5$).}
\label{tab:crossunit}
\footnotesize
...
Dense            & .36 & .66 & .44 & .66 \\
Hybrid           & .37 & .68 & .45 & .69 \\
String norm.     & .34 & .79 & .46 & .70 \\
LLM-assisted     & .38 & .70 & .47 & .71 \\
\midrule
\rowcolor{gray!10}
\textbf{Ours}    & \textbf{.97} & \textbf{1.00} & \textbf{.98} & \textbf{1.00} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!t]
\centering
\caption{Ablation on cross-unit queries.}
\label{tab:ablation}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}llcc@{}}
\toprule
\textbf{Config.} & \textbf{Components} & \textbf{F1@5} & \textbf{$\Delta$} \\
\midrule
A & BM25 only                  & 0.42 & --- \\
B & A + dense vectors          & 0.45 & +.03 \\
\midrule
\rowcolor{gray!10}
C & B + sci.\ operators        & \textbf{0.98} & \textbf{+.53} \\
D & C + graph traversal        & 0.98 & +.00 \\
E & D + agentic rewriting      & 0.98 & +.00 \\
F & Full pipeline              & 0.98 & +.00 \\
\bottomrule
\end{tabular}
\end{table}
