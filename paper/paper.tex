...
We evaluate CLIO along three axes: (1)~agentic efficiency---does
wrapping the retrieval pipeline behind a single tool call reduce LLM
token consumption and latency compared to raw catalog access?
(2)~science-aware accuracy---do dimensional conversion and formula
normalization improve retrieval precision on cross-unit queries where
text-based methods fail? (3)~storage integration---can CLIO serve as
the search layer for IOWarp's tiered blob storage, enabling unit-aware
queries that the native BlobQuery API cannot express?
...
Hopper nodes). IOWarp experiments run inside Docker with the
\texttt{iowarp/deps-cpu} image and Chimaera runtime v1.0.3.

\textbf{Controlled benchmark.} 210 documents across seven scientific
domains (Table~\ref{tab:dataset}), each containing realistic prose
with deliberate unit heterogeneity: the same physical quantities are
expressed in different SI prefixes across documents. We define 80
queries with automatically generated ground truth.
...
\textbf{Baselines.} We compare five retrieval configurations:
(1)~BM25-only (DuckDB full-text search); (2)~Dense-only
(all-MiniLM-L6-v2, cosine similarity); (3)~Hybrid BM25+Dense (linear
fusion, equal weights); (4)~String normalization, following the Numbers
Matter! approach~\cite{numbersmatter2024}; and (5)~the full CLIO
pipeline with dimensional conversion, formula normalization, and
agentic rewriting.

\textbf{Metrics.} Precision at $K$ (P@$K$), recall at $K$ (R@$K$),
and mean reciprocal rank (MRR) at $K{=}5$. For the agentic efficiency
experiment, we measure total tokens (including sub-agent tokens) and
wall-clock time.

%% ----- 5.2: Agentic Efficiency -----
\subsection{Agentic Efficiency}
\label{sec:agentic-efficiency}

We run 10~scientific discovery queries through three paths using the
same LLM (\texttt{claude-sonnet-4-20250514}) and the same National
Data Platform (NDP) catalog:

\begin{itemize}
\item \textbf{Claude Native Agent:} No domain-specific tools. The
  agent uses web search and spawns sub-agents for parallel research
  across NOAA, NASA, and NDP repositories.
\item \textbf{NDP-MCP:} The agent accesses NDP via the Model Context
  Protocol, calling \texttt{search\_datasets} and
  \texttt{get\_dataset\_details} directly. Raw catalog JSON enters
  the LLM context window.
\item \textbf{CLIO:} The agent calls a single \texttt{clio\_search}
  tool. CLIO internally connects to NDP-MCP, indexes 341~datasets
  (359~chunks, 1{,}945~measurements) into DuckDB, and returns
  $\sim$2K tokens of ranked citations per query.
\end{itemize}

\begin{table}[t]
\centering
\caption{Agentic efficiency on 10~scientific data discovery queries.
All three paths use \texttt{claude-sonnet-4-20250514} and query the
same NDP catalog. $\Delta$~is relative to the Claude Native Agent.}
...
consumption by \textbf{52.08\%} and wall-clock time by
\textbf{91.27\%} compared to the native agent baseline, while answering
all 10~queries. The native agent and NDP-MCP consume nearly identical
tokens ($\sim$989K) through different mechanisms: the native agent
spawns 9~sub-agents for parallel web research (563K main + 426K
sub-agent tokens), while NDP-MCP injects raw catalog JSON into the LLM
context (989K main tokens). CLIO avoids both by keeping catalog text
in a local index and returning only compact ranked citations.

NDP-MCP reduces wall time by 65.68\% over the native agent
(17.4~min vs.\ 50.8~min) by querying the API directly rather than
searching the web, but achieves no token savings because the catalog
text still enters the prompt. CLIO reduces time by a further 74.57\%
over NDP-MCP (4.4~min vs.\ 17.4~min) because the agent receives
10~ranked citations per query rather than 25~full dataset descriptions.
...
across 10~queries. CLIO achieves 52\% token reduction and 91\% time
reduction vs.\ the native agent.}
\label{fig:l1-three-path}
\end{figure}

%% ----- 5.3: Cross-Unit Retrieval Accuracy -----
\subsection{Cross-Unit Retrieval Accuracy}

We evaluate whether dimensional conversion improves retrieval accuracy
when query and document units differ.
...
\subsection{Formula Matching Accuracy}

We evaluate whether formula normalization improves retrieval accuracy
on queries involving mathematical notation variants.
...
\subsection{Federated Search Coverage}

We evaluate whether federated multi-namespace search maintains
retrieval coverage when the same corpus is distributed across
...
\subsection{IOWarp Blob Storage Integration}
\label{sec:iowarp}

At IOWarp's target scale of 10~billion blobs, agents need to determine
\emph{what to retrieve} before fetching. IOWarp's native BlobQuery API
returns all blobs matching a tag-name regex with no content filtering;
at 10B blobs this is unusable. CLIO addresses this by indexing
scientific metadata from IOWarp CTE blobs at ingest time, then
answering unit-aware queries against the index.

We run the IOWarp Chimaera runtime (v1.0.3) inside Docker and store
synthetic scientific blobs across four CTE tags (temperature in
\textdegree C, pressure in kPa, wind in m/s, humidity in \%). Four
cross-unit queries use deliberately mismatched units: temperature in
\textdegree F, pressure in psi, wind in km/h.

\begin{table}[t]
\centering
\caption{CLIO vs.\ raw BlobQuery on IOWarp CTE blobs. CLIO returns
top-50 unit-converted matches; BlobQuery returns all blobs in the tag
with no filtering. All four cross-unit queries succeed at every scale.}
\label{tab:iowarp}
\small
...
Table~\ref{tab:iowarp} shows three results. First, CLIO finds
cross-unit matches at every scale while BlobQuery cannot perform any
content-level filtering. Second, corpus profiling completes in
14--22\,ms via DuckDB aggregation. Third, CLIO's inspection rate falls
from 5.0\% at 1K blobs to \textbf{0.5\% at 10K}: rather than
returning all 2{,}500 blobs in a tag, CLIO returns 50 matching blobs
($50\times$ reduction). At 10B blobs, this selectivity eliminates
brute-force scanning entirely.

\subsection{Distributed Scaling}
\label{sec:distributed-scaling}

We evaluate CLIO's coordinator--worker pipeline on NCSA DeltaAI
(NVIDIA GH200 Grace Hopper nodes) with 2.5~million arXiv abstracts
pre-indexed into four DuckDB shards of 625K documents each. We measure
query latency across 8~query types $\times$ 10~repeats = 80
measurements per configuration.

\textbf{Strong scaling.}
All four shards are available to every configuration; we vary only the
number of active workers (1, 2, 4).

\begin{table}[t]
\centering
\caption{Strong scaling: query latency (ms) at fixed 2.5M-document
...
Table~\ref{tab:strong-scaling} shows p50 latency remains sub-42\,ms
across all configurations. The key result is \emph{sustained low
latency at scale}: all 80~queries complete under 70\,ms with p50 under
42\,ms across 2.5~million documents, regardless of worker count. The
flat scaling profile indicates that DuckDB query time on each shard is
already fast enough that coordinator fan-out dominates.

\textbf{Weak scaling.}
Data and workers grow together: 1~worker / 625K docs, 2~workers /
1.25M docs, 4~workers / 2.5M docs.

\begin{table}[t]
\centering
\caption{Weak scaling: latency (ms) as data and workers grow together.
Efficiency = $\text{p50}_{1\text{w}} / \text{p50}_{N\text{w}}$.}
...
Table~\ref{tab:weak-scaling} shows weak scaling efficiency of
\textbf{0.94} at $4\times$ scale: quadrupling both data and workers
increases p50 latency by only 2.5\,ms (6.5\%). This near-linear weak
scaling demonstrates that CLIO's architecture supports horizontal
scale-out with minimal overhead.
...
\subsection{Metadata-Adaptive Strategy Selection}
\label{sec:adaptive}

We evaluate CLIO's ability to adapt when datasets vary in metadata
quality. We create three corpus variants from 210~documents:
\textbf{rich} (87.5\% metadata density, 2{,}674 measurements),
\textbf{sparse} (measurements stripped, 10.3\% density), and
\textbf{no-metadata} (all numbers removed, 4.4\% density). CLIO
correctly classifies each variant and selects the appropriate retrieval
strategy: rich $\to$ scientific + lexical + vector, sparse $\to$
lexical + vector (scientific skipped), no-metadata $\to$ lexical +
vector only. On 10~cross-unit queries, only the rich corpus produces
hits via SI conversion; the other two correctly fall back to text-based
retrieval, confirming that unit conversion activates only when metadata
exists.

\subsection{Real-World Validation on NOAA Observatory Data}
\label{sec:real-world}

To validate that controlled-benchmark findings generalize, we evaluate
on 288~real documents from the NOAA Global Historical Climatology
Network --- Daily (GHCN-D) \cite{ghcn2012}, covering 8~U.S.~stations
across 2022--2024. Documents report metric units only (degC, mm, m/s);
8~cross-unit queries use imperial units (\textdegree F, inches, mph).

\begin{table}[!t]
\centering
\caption{Real-world evaluation on NOAA GHCN-Daily (288~docs, 8~stations).
Documents are metric-only; queries use imperial units.}
...
Table~\ref{tab:real_world} confirms the controlled-benchmark findings
on real observatory data. Baselines achieve P@5 $\le$ 0.375 because no
text-based method can match ``86~degrees~Fahrenheit'' against
``30.0~degC.'' CLIO reaches \textbf{0.700~P@5 (+87\%)} by
canonicalizing both query and document measurements to Kelvin.

\subsection{Error Analysis}
\label{sec:errors}

We catalog the failure modes of the science-aware operators.
\textbf{Extraction failures:} the regex extractor misses non-standard
formats (e.g., ``$2 \times 10^3$~Pa''), achieving 95\% precision and
90\% recall on measurement identification. \textbf{Unit ambiguity:}
``m'' may denote meters or mass; resolved by requiring a numeric literal
adjacent to the unit token. \textbf{Unsupported units:} queries
involving compound units (N/m$^2$, mol/L) produce no dimensional
matches and fall back to lexical retrieval. The 46-unit registry
(12~SI domains) can be extended by adding rows without code changes.
