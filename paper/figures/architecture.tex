...
\node[connbox, right=1.1cm of ns, yshift=0.65cm]  (fs)   {Filesystem};
\node[connbox, below=0.12cm of fs]                 (s3)   {S3 Object Store};
\node[connbox, below=0.12cm of s3]                 (qd)   {Qdrant};
\node[connbox, below=0.12cm of qd]                 (neo)  {Neo4j};
\node[sciconn, below=0.12cm of neo]                (hdf)  {HDF5};
\node[sciconn, below=0.12cm of hdf]                (nc)   {NetCDF};

% label
\node[above=0.08cm of fs, font=\tiny\bfseries] {Connectors};
...
\begin{pgfonlayer}{background}
  \node[draw=black!30, dashed, rounded corners=3pt, fill=black!3,
        inner sep=4pt,
