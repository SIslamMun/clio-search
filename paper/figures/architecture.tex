...
\begin{tikzpicture}[
  ...
  connbox/.style={
    draw, rounded corners=2pt, minimum height=0.35cm,
    text centered, inner sep=1.5pt, fill=white,
    minimum width=1.4cm, font=\tiny
  },
  sciconn/.style={
...
% ====== Right side: connectors (two columns) ======
% Column 1: general connectors
\node[connbox, right=0.9cm of ns, yshift=0.55cm]   (fs)   {Filesystem};
\node[connbox, below=0.06cm of fs]                  (s3)   {S3};
\node[connbox, below=0.06cm of s3]                  (qd)   {Qdrant};
\node[connbox, below=0.06cm of qd]                  (neo)  {Neo4j};

% Column 2: science-aware connectors
\node[sciconn, right=0.08cm of fs]                  (hdf)  {HDF5};
\node[sciconn, below=0.06cm of hdf]                 (nc)   {NetCDF};
\node[sciconn, below=0.06cm of nc]                  (iow)  {IOWarp};
\node[connbox, below=0.06cm of iow]                 (ndp)  {NDP};

% label
\node[above=0.06cm of fs, font=\tiny\bfseries, xshift=0.7cm] {Connectors (9)};

% arrows from connectors to namespace registry
\draw[arr] (fs.west)  -- (ns.east |- fs.west);
\draw[arr] (s3.west)  -| ([xshift=0.12cm]ns.east) -- (ns.east);
\draw[arr] (qd.west)  -| ([xshift=0.12cm]ns.east) -- (ns.east);
\draw[arr] (neo.west) -| ([xshift=0.12cm]ns.east) -- (ns.east);
\draw[arr] (hdf.west) -- (fs.east);
\draw[arr] (nc.west)  -- (s3.east);
\draw[arr] (iow.west) -- (qd.east);
\draw[arr] (ndp.west) -- (neo.east);
...
(right) mediate heterogeneous backends; science-aware components (shaded)
execute alongside standard retrieval branches. The agentic loop (left,
dashed) iteratively refines queries via LLM rewriting.}
