#!/bin/bash
# v6 — like v5 but I/O instrumentation is now AROUND THE AGENT'S PYTHON SCRIPT
# only, not around opencode itself. Uses /home/sislam6/jarvis-work/io_wrap.py
# which reads /proc/self/io before+after the wrapped script and prints
# IO_DELTA: {...} as its last line.
...
if [ "$DATASET" = "era5" ] && [ "$ARM" = "a" ]; then
    echo "[rank $RANK] arm=A on ERA5 not implemented; exit 0" >&2
    exit 0
fi
...
case "$DATASET" in
    argo)
        DATA_DIR_OVERRIDE="/lus/flare/projects/gpu_hack/sislam6/clio_eval/big_corpus/data"
        INDEX_PATH="/lus/flare/projects/gpu_hack/sislam6/clio_eval/big_corpus/argo_big_index.duckdb"
        BLOBS_JSON="/lus/flare/projects/gpu_hack/sislam6/clio_eval/big_corpus/argo_big_blobs.json"
        NS="agent_test_argo_big"
        TAG_PATTERN="argo_.*"
    ...
    era5)
        ...
        BLOBS_JSON="/tmp/era5_blobs_unified.json"
        NS="agent_test_era5"
        TAG_PATTERN="era5_.*"
...
IOWRAP=/home/sislam6/jarvis-work/io_wrap.py
...
# ---- Build TASK — agent's apptainer command now wraps python3 with io_wrap ----

TASK_A="You MUST do TWO things in order: (1) Write a Python script ./scan.py that reads the NetCDF files in data/ using the netCDF4 module and counts ${PROMPT_NL_A_VAL}. (2) IMMEDIATELY EXECUTE the script via the apptainer command and report the integer it prints. Both steps required. \
\
After writing the file, RUN this exact command and read the output: \
apptainer exec --bind /lus:/lus /home/sislam6/jarvis-work/sif/iowarp_deploy_cpu.sif bash -c 'source /home/sislam6/jarvis-work/iowarp-venv/bin/activate && python3 /home/sislam6/jarvis-work/io_wrap.py ./scan.py' \
\
...
\
After writing the file, RUN this exact command and read the output: \
apptainer exec --bind /lus:/lus /home/sislam6/jarvis-work/sif/iowarp_deploy_cpu.sif bash -c 'source /home/sislam6/jarvis-work/iowarp-venv/bin/activate && python3 /home/sislam6/jarvis-work/io_wrap.py ./q.py' \
\
Print only the integer it prints, on the very last line of your final response."
...
# Extract IO_DELTA: {...} from the agent.log (printed by io_wrap.py)
IO_DELTA=$(grep -oE 'IO_DELTA: \{[^}]+\}' $LOG | tail -1 | sed 's/^IO_DELTA: //')
[ -z "$IO_DELTA" ] && IO_DELTA="null"
