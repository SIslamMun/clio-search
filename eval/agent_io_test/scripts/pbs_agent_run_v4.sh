...
# DATA_DIR depends on DATASET (raw NetCDFs the naive arm sees)
case "$DATASET" in
...
RUN_DIR=$BASE/results/v4_${DATASET}_${RUN_TS}
mkdir -p $RUN_DIR
lfs setstripe -p gecko.ddn_hdd -E -1 -c 2 $RUN_DIR 2>/dev/null

NUM_NODES=$(wc -l < $PBS_NODEFILE)
echo "v4 dataset=$DATASET arm=$ARM nodes=$NUM_NODES run_dir=$RUN_DIR"

T0=$(date +%s)
export RUN_DIR ARM DATASET DATA_DIR BASE
mpiexec -n $NUM_NODES -ppn 1 $BASE/scripts/agent_inner_v4.sh
T1=$(date +%s)
