#!/bin/bash
# ---------------------------------------------------------------------------
# L2-C LARGE: CLIO + IOWarp NO METADATA — 500K and 1M scales (native build)
#
...
ALL_RESULTS_FILE="/tmp/l2c_large_all_results_${SLURM_JOB_ID}.json"
...
echo "Node: $(hostname)"
echo "Date: $(date)"
echo "Job: L2-C large (500K, 1M, no metadata, one process per scale)"
echo "Scales: $SCALES"
echo "========================================"
...
for N in $SCALES; do
    chimaera_cleanup
    RAW_FILE="/tmp/l2c_large_scale_${N}_${SLURM_JOB_ID}.txt"
    echo ""
    echo "========================================"
